...
public class JPQLTest {
	...
	public void jpql_courses_without_students() {
		...
		List<Course> resultList = query.getResultList();
		logger.info("Results -> {students is empty}", resultList);
	...
	public void jpql_courses_with_atleast_2_students() {
		...
		List<Course> resultList = query.getResultList();
		logger.info("Results -> {student size >= 2}", resultList);
	...
	public void jpql_courses_ordered_by_students() {
		...
		List<Course> resultList = query.getResultList();
		logger.info("Results -> {students order by}", resultList);
	}
	...
	public void jpql_students_with_passports_in_a_certain_pattern() {
		...
		logger.info("Results -> {student, pasport like}", resultList);
	}
		...
		public void join(){
			...
			logger.info("Results Size -> {JOIN}", resultList.size());
		...
		public void left_join(){
			...
			List<Object[]> resultList = query.getResultList();
			logger.info("Results Size -> {LEFT JOIN}", resultList.size());
			for(Object[] result:resultList){
				logger.info("Course{} Student{}", result[0], result[1]);
			}
		}
		...
		public void cross_join(){
			...
			List<Object[]> resultList = query.getResultList();
			logger.info("Results Size -> {CROSS}", resultList.size());
			for(Object[] result:resultList){
				logger.info("Course{} Student{}", result[0], result[1]);
			}
		}
